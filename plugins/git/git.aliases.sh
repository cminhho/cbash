...
gcpd() { git checkout "$(get_default_branch)"; git pull; git branch -D "$1"; }
gcobu() { git checkout -b "${USER}/$1"; }

# --- Core ---
alias g='git'

# --- CBASH subcommands (delegate to plugin) ---
...
alias gp='git pull origin $(get_default_branch)'
alias gl='git pull'
...
gdv() { git diff --ignore-all-space "$@" | vim -R -; }
...
alias gtl='git tag -l'
case "$OSTYPE" in
    darwin*) alias gtls="git tag -l | gsort -V" ;;
...
# --- CBASH shortcuts (plugin commands) ---
